...
"""Kick subscription, moderation, pin and host parser contracts."""
...
@pytest.mark.parametrize(
    "payload",
    [None, {}, 3.14, "not a dict", 42, [], ""],
)
def test_event_parsers_reject_invalid_payloads(parser, payload):
    with pytest.raises(ParsingError):
        parser(payload)


@pytest.mark.parametrize("created_at", ["", "not-a-date", 123])
def test_event_parsers_omit_invalid_timestamps(parser, created_at):
    assert "timestamp" not in parser({"id": "x", "created_at": created_at})


def test_event_parsers_missing_optional_fields(parser):
    kind = parser.__name__.removeprefix("parse_").removesuffix("_event")
    if kind == "pinned_message_created":
        kind = "pinned_message"
    msg = parser({"id": "x"})
    assert msg["message_id"] == "x"
    assert msg["message_type"] == kind
    assert msg["message"] == ""
    assert not {"timestamp", "author", "metadata"} & msg.keys()
...
def test_numeric_id_coercion(parser, raw_id, expected):
...
def test_standard_event_fixtures(parser, kind, message_id, text, author, metadata):
    ...
    assert (msg["message_id"], msg["message_type"], msg["message"]) == (
        message_id,
        kind,
        text,
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            parse_user_banned_event,
            load_fixture("user_banned_event_temporary.json"),
            {
        ...
        (
            parse_message_deleted_event,
            load_fixture("message_deleted_event_ai.json"),
            {
        ...
        (
            parse_message_deleted_event,
            {"id": "x", "aiModerated": False, "violatedRules": ["valid", 7, None]},
            {"ai_moderated": False, "violated_rules": ["valid"]},
        ),
        (parse_message_deleted_event, {"id": "x", "violatedRules": "hate"}, None),
        (
            parse_user_banned_event,
            {"id": "x", "expires_at": "2024-01-01T00:01:00Z"},
            {"expires_at": 1704067260000000},
        ),
        (parse_user_banned_event, {"id": "x", "expires_at": ""}, {"expires_at": ""}),
        *[
            (parse_user_banned_event, {"id": "x", field: value}, None)
            for field, value in [
                ("duration", True),
                ("duration", -1),
                ("permanent", 1),
...
def test_moderation_metadata(parser, raw, metadata):
    msg = parser(raw)
    if metadata is None:
...
def test_pinned_message_fixtures(
    ...
    assert metadata["pinned_message_id"] == pinned_id
    assert metadata["original_message_created_at"] == created
    assert metadata["pinned_message_created_at"] == created
    assert metadata["duration"] == duration
    if timestamp is None:
        assert "timestamp" not in msg
        assert metadata["pinned_by"]["display_name"] == "PinningModerator"
    else:
        assert isinstance(msg["timestamp"], int)
        assert msg["timestamp"] == timestamp
        assert "pinned_by" not in metadata


def test_pinned_message_created_rejects_empty_nested_id():
    with pytest.raises(ParsingError):
        parse_pinned_message_created_event({"message": {"id": ""}})


def test_pinned_message_deleted_uses_namespaced_nested_id_fallback():
...
def test_pinned_message_omits_invalid_nested_timestamp():
    msg = parse_pinned_message_created_event(
        {"id": "x", "message": {"id": "pinned", "created_at": "not-a-date"}}
    )
    assert not {"original_message_created_at", "pinned_message_created_at"} & (
        msg["metadata"].keys()
    )
