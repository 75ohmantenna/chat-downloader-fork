...
_ACTION_HANDLERS: dict[str, _ActionHandler] = {
    action_type: handler
    for types, handler in (
        (_KNOWN_ITEM_ACTION_TYPES, _handle_item_action),
        (_KNOWN_INTERACTIVITY_WIDGET_ACTION_TYPES, _handle_interactivity_widget_action),
...
def process_action(
    ...
    handler = _ACTION_HANDLERS.get(original_action_type)
    if handler is not None:
        return ProcessedAction(*handler(action, original_action_type, data, offset))
